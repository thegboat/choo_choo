...
describe 'edi 835 validation' do
  ...
  it 'should have no errors' do
    edi = ChooChoo::Parser.parse_835(test_message)
  ...
  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_ISA_SEGMENT" do
    ...
    isa = edit_line(test_message, 1, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_ISA_SEGMENT")).to eq(true)
  end
  ...
  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_BPR_SEGMENT" do
    ...
    isa = edit_line(test_message, 4, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_BPR_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_TRN_SEGMENT" do
    string = "TRN*1*WM0009430876*1710226428*USWM* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 5, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_TRN_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_DTM_SEGMENT" do
    string = "DTM*405*20150102* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 7, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_DTM_SEGMENT")).to eq(true)
  end
  ...
  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_PER_SEGMENT" do
    ...
    isa = edit_line(test_message, 12, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_PER_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_REF_SEGMENT" do
    string = "REF*F2*HCFA1500* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 6, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_REF_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_CAS_SEGMENT" do
    string = "CAS*PR*1*89.55* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 26, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_CAS_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_IEA_SEGMENT" do
    string = "IEA*0*000000000* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 266, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_IEA_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_SVC_SEGMENT" do
    string = "SVC*HC<99202<25*110*0* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 24, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_SVC_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_CLP_SEGMENT" do
    string = "CLP*095950951285*1*215*0*135.82*12*141230N11B94* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 19, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_CLP_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_LX_SEGMENT" do
    string = "LX*1* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 18, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_LX_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_LQ_SEGMENT" do
    string = "LQ*HE*N130* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 111, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_LQ_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_N1_SEGMENT" do
    string = "N1*PR*ABCBS DBA BLUEADVANTAGE ADMIN OF AR* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 8, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_N1_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_N3_SEGMENT" do
    string = "N3*P.O. BOX 1460* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 9, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_N3_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_N4_SEGMENT" do
    string = "N4*LITTLE ROCK*AR*72203* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 10, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_N4_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_GS_SEGMENT" do
    string = "GS*HP*133052274*264007676*20150105*0451*1*X*005010X221A1* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 2, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_GS_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_ST_SEGMENT" do
    string = "ST*835*0006* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 3, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_ST_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_GE_SEGMENT" do
    string = "GE*0*00* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 265, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_GE_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_SE_SEGMENT" do
    string = "SE*00*000000000* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 264, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_SE_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_NM1_SEGMENT" do
    string = "NM1*QC*1*WALKER*LANDON*M***MI*WMB09180949W* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 20, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_NM1_SEGMENT")).to eq(true)
  end

  it "handles WRONG_NUMBER_OF_ELEMENTS_FOR_AMT_SEGMENT" do
    string = "AMT*B6*89.55* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * "
    isa = edit_line(test_message, 31, string)
    expect(isa.errors.include?("WRONG_NUMBER_OF_ELEMENTS_FOR_AMT_SEGMENT")).to eq(true)
  end
end